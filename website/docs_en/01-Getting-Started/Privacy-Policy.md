...
# Inimark Privacy Policy

This privacy policy describes how Inimark ("the App") collects, uses, and shares user information.  
...
**Inimark is a local-first desktop application and does not actively upload any of your information to any server.**  
Specifically:
...
- This request sends **only the app version and current OS** — no personal data. GitHub may record standard web access logs (IP, UA) — this is standard GitHub service behavior, unrelated to Inimark.
...
- You may use cloud sync (e.g., OneDrive, iCloud, Dropbox) or version control (e.g., Git) to protect files at your discretion — **Inimark does not intervene or intercept these syncs.**
...
- Deletion methods:
  ...
  - App preferences: Uninstall the app or reset in app settings; or manually delete config files under `%APPDATA%/com.inimark.editor/` (Windows).
