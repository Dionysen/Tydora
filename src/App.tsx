...
function App({ initialFilePath, initialVaultPath }: { initialFilePath?: string | null; initialVaultPath?: string | null }) {
  ...
  const commands = useMemo(() => [
    ...
    { id: "settings-editor", label: t("settings.tabs.editor"), category: t("app.command.categories.settings"), aliases: ["编辑器", "editor", "打字机", "保存", "格式化"].flatMap((s) => s.split(", ")), action: () => { localStorage.setItem("inimark-settings-initial-tab", "editor"); invoke("open_settings_window"); } },
    { id: "settings-markdown", label: t("settings.tabs.markdown"), category: t("app.command.categories.settings"), aliases: ["Markdown", "markdown", "WikiLink", "Mermaid", "Callout", "数学"].flatMap((s) => s.split(", ")), action: () => { localStorage.setItem("inimark-settings-initial-tab", "markdown"); invoke("open_settings_window"); } },
    { id: "settings-appearance", label: t("settings.tabs.appearance"), category: t("app.command.categories.settings"), aliases: ["外观", "appearance", "界面", "侧栏", "顶栏"].flatMap((s) => s.split(", ")), action: () => { localStorage.setItem("inimark-settings-initial-tab", "appearance"); invoke("open_settings_window"); } },
