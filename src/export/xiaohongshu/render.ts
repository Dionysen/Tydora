...
import type { XhsSettings, XhsTheme, XhsCard, XhsRatio } from "./types";
...
.xhs-card { position: relative; overflow: hidden; box-sizing: border-box; font-family: Optima-Regular, Optima, PingFangSC-light, PingFangTC-light, "PingFang SC", sans-serif; }
...
.xhs-card .xhs-card-grid-overlay { position: absolute; top: 0; left: 0; right: 0; bottom: 0; pointer-events: none; z-index: -1; background-image: linear-gradient(to right, rgba(120, 120, 120, 0.10) 1px, transparent 1px), linear-gradient(to bottom, rgba(120, 120, 120, 0.10) 1px, transparent 1px); }
`;
...
function buildFooter(settings: XhsSettings, index: number, total: number): HTMLElement {
  ...
  footer.style.cssText = `position:absolute;left:0;right:0;bottom:0;height:${FOOTER_HEIGHT}px;padding:0 ${CONTENT_PADDING_X}px;box-sizing:border-box;border-top:1px solid var(--border, #2c2c2e);background:var(--bg-primary);`;
...
export async function buildXhsCards(opts: {
  getContentElement: () => HTMLElement | null;
  editorTheme: string;
  settings: XhsSettings;
  title: string;
}): Promise<XhsBuildResult> {
  ...
  try {
    ...
    // 编辑器图片节点外层包着 .image-node-view（inline-block），若只处理 img 本身，
    // 该包裹层仍按行内元素靠左排列，导致图片不居中，故需连同包裹层一并块级化居中。
    ...
    // 加粗等文本样式显式内联，保持与文档渲染一致：html-to-image 只复制文档
    // 样式表与内联样式，UA 默认的 <strong> 粗体在 foreignObject 中不可靠，
    // 故将 font-weight/color 直接内联到元素上兜底（测量容器与卡片克隆共用）。
    ...
    try {
      ...
      if (useCoverCard) {
        const coverEl = buildCoverCard(theme, opts.settings, coverInfo, 0, totalPages);
        ...
        jobs.push({
          ...
          index: 0,
          isCover: true,
          cacheKey: `${settingsKey}|cover|${opts.settings.pageNumber ? `0/${totalPages}` : ""}|${cyrb53(coverInfo.title)}`,
          from: 0,
          to: 0,
        });
      }
      for (let i = 0; i < bodyPages; i++) {
        ...
        const el = buildBodyCard(contentEl, breaks[i], breaks[i + 1], theme, opts.settings, idx, totalPages);
        host.appendChild(el);
        jobs.push({
          el,
        ...
        for (;;) {
          ...
          if (!job.isCover) {
            ...
            if (contentClone) {
              ...
              const pageKey = opts.settings.pageNumber ? `${job.index}/${totalPages}` : "";
              cacheKey = `${settingsKey}|page|${pageKey}|${cyrb53(hashEl.outerHTML)}`;
            }
          }
