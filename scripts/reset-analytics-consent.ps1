<#
.SYNOPSIS
    重置 Inimark 的匿名统计同意状态，让「首次启动同意弹窗」重新出现。
.DESCRIPTION
    - 自动关闭正在运行的 Inimark（dev 与 release 均适用）
    - 备份并删除 WebView2 的 Local Storage 目录（里面包含 inimark.analytics.consent）
    - 重新启动 Inimark 后即可看到同意弹窗
...
Write-Host "==> [1/4] 关闭正在运行的 Inimark ..."
Get-Process -Name "inimark", "Inimark" -ErrorAction SilentlyContinue | Stop-Process -Force
Start-Sleep -Milliseconds 1000

# WebView2 的 Local Storage 目录（Tauri v2 + WebView2 默认位置）
$storageDir = Join-Path $env:LOCALAPPDATA "com.inimark.editor\EBWebView\Default\Local Storage"

if (-not (Test-Path $storageDir)) {
    Write-Host "    未找到 Local Storage 目录：$storageDir"
    Write-Host "    可能从未运行过 Inimark，或路径有差异。请手动检查后重试。"
    exit 1
}
...
if (-not (Test-Path $storageDir)) {
    ...
    Write-Host "    删除失败：目录仍存在，可能有进程占用文件。请关闭 Inimark 后重试。"
    exit 1
}

Write-Host "==> [4/4] 完成！"
Write-Host ""
Write-Host "    现在重新启动 Inimark（例如：npm run tauri dev），即可看到首次启动的同意弹窗。"
